  ...
  if (cart.length === 0) {
    return (
      <div className="max-w-lg mx-auto flex flex-col items-center justify-center min-h-[60vh] px-6 text-center">
        ...
        <h2 className="text-white font-['Fraunces'] text-xl mb-2">Your cart is empty</h2>
  ...
  return (
    <div className="max-w-lg mx-auto px-4 pt-5 pb-6">
      ...
      <div className="space-y-3">
        {cart.map((item) => (
          <div key={`${item.product.id}-${item.selectedColor}`} className="flex gap-3 bg-[#111111] rounded-2xl border border-[#1c1c1c] p-3">
            <div className="relative w-20 h-20 rounded-xl overflow-hidden bg-[#0e0e0e] shrink-0">
              <Image src={item.product.variants.find((v: any) => v.color === item.selectedColor)?.image || item.product.variants[0].image} alt={item.product.name} fill sizes="80px" className="object-cover" />
            </div>

            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-[#555] text-[9px] tracking-widest uppercase">{item.product.brand}</p>
                  <p className="text-white font-['Fraunces'] text-sm leading-tight">{item.product.name}</p>
                  <p className="text-[#555] text-[10px] mt-0.5">{item.selectedColor}</p>
                </div>
                <button
                  onClick={() => dispatch(removeFromCart({ productId: item.product.id, color: item.selectedColor }))}
              ...
              <div className="flex items-center justify-between mt-2.5">
                <div className="flex items-center gap-2 border border-[#222] rounded-xl overflow-hidden">
                  <button
                    onClick={() => dispatch(updateQuantity({ productId: item.product.id, color: item.selectedColor, quantity: item.quantity - 1 }))}
                ...
                <p className="text-white font-semibold text-sm">PKR {(item.product.price * item.quantity).toFixed(2)}</p>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="mt-5 bg-[#111111] rounded-2xl border border-[#1c1c1c] p-4">
        <p className="text-[#555] text-[10px] tracking-[0.3em] uppercase font-medium mb-4">Order Summary</p>
        <div className="space-y-2.5">
          ...
          <div className="border-t border-[#1c1c1c] pt-2.5 flex justify-between">
            ...
            <span className="text-white font-semibold text-lg">PKR {total.toFixed(2)}</span>
          </div>
        </div>
  ...
  Checkout
      ...
      <div className="grid grid-cols-3 gap-2 mt-4">
        {[
          ...
          <div key={badge.label} className="flex flex-col items-center gap-1 py-3 bg-[#0e0e0e] rounded-xl border border-[#1a1a1a]">
            <span className="text-lg">{badge.icon}</span>
            <span className="text-[#555] text-[10px] tracking-wide">{badge.label}</span>
          </div>
        ))}
      </div>
    </div>
