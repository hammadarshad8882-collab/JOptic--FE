...
export default function ProductTab({ setTab, onDeleteProduct }: ProductTabProps) {
 ...
 const handleDelete = async(id: string) => {
  ...
  try {
    ...
    if (
      ...
      formData.append(
        "originalPrice",
        String(productToUpdate.originalPrice)
      );
    }
    ...
    formData.append(
      "frameShape",
      productToUpdate.frameShape || ""
    );

    formData.append(
      "frameMaterial",
      productToUpdate.frameMaterial || ""
    );

    formData.append(
      "lensWidth",
      productToUpdate.lensWidth || ""
    );

    formData.append(
      "bridgeWidth",
      productToUpdate.bridgeWidth || ""
    );

    formData.append(
      "templeLength",
      productToUpdate.templeLength || ""
    );

    formData.append(
      "isNew",
      String(productToUpdate.isNew)
    );

    formData.append(
      "isBestseller",
      String(productToUpdate.isBestseller)
    );
    ...
    <div>
      <div className="flex items-baseline justify-between mb-4">
        <div>
          ...
          <p className="text-[#444] text-xs mt-0.5">{products.length} items in catalogue</p>
        </div>
        <button
          onClick={() => setTab('add')}
      ...
      <div className="space-y-3">
        {filteredProducts.map((product) => (
          <div key={product.id} className="bg-[#0e0e0e] border border-[#1a1a1a] rounded-2xl overflow-hidden hover:border-[#2a2a2a] transition-all">
            <button
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-12 h-12 rounded-lg overflow-hidden bg-[#111] shrink-0">
                  <img src={product.variants[0].image} alt={product.name} className="w-full h-full object-cover" />
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    <span className="text-[#444] text-[10px]">{product.category}</span>
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-4 shrink-0">
                <div className="text-right">
                  <p className="text-white text-sm font-semibold">PKR{product.price}</p>
                  {product.isNew && (
                    <span className="text-[#81c784] text-[9px] tracking-wide uppercase">New</span>
                  )}
                  {product.isBestseller && (
                    <span className="ml-1 text-[#f0b429] text-[9px] tracking-wide uppercase">Best</span>
                  )}
                </div>
                <svg
                  width="14"
            ...
            {expandedProduct === product.id && (
              <div className="border-t border-[#1a1a1a] p-4 space-y-4">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {[
                    { label: 'Original Price', value: product.originalPrice ? `PKR${product.originalPrice}` : '-' },
                ...
                <div>
                  <p className="text-[#333] text-[10px] uppercase tracking-wide mb-1">Description</p>
                  <p className="text-[#aaa] text-xs leading-relaxed">{product.description}</p>
                </div>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-end justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setDeleteConfirm(null)}>
          <div className="bg-[#111] border border-[#2a2a2a] rounded-2xl p-5 w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
            <p className="text-white font-['Fraunces'] text-lg mb-1">Delete product?</p>
            <p className="text-[#555] text-sm mb-5">This will remove it from the catalogue. This cannot be undone.</p>
            <div className="flex gap-3">
              <button onClick={() => setDeleteConfirm(null)} className="flex-1 py-3 border border-[#222] text-[#666] text-sm rounded-xl hover:border-[#333]">Cancel</button>
              <button
                onClick={() => { onDeleteProduct(deleteConfirm); setDeleteConfirm(null); }}
    ...
    >
      ...
      {filteredProducts.length === 0 && isloading === false && (
        <div className="text-center py-16">
          <p className="text-[#333] font-['Fraunces'] text-lg">No products found</p>
        </div>
      )}
    </div>
  );
